...
void autonomous() {
  ...
  chassis.MotionController.waitUntilSettled();
  chassis.MotionController.setTarget("1f_Collect_Cubes_1", false);  //Drive forward to collect dem' cubes
  chassis.MotionController.generatePath({Point{0_in,0_in,0_deg}, Point{12_in,0_in,0_deg}}, "1f_Collect_Cube_6");
  chassis.MotionController.waitUntilSettled();

  pros::delay(2000);

  chassis.MotionController.setTarget("1f_Collect_Cube_6", false);  //Drive forward to collect dem' cubes
  chassis.MotionController.generatePath({Point{0_in,0_in,0_deg}, Point{5_in,0_in,0_deg}}, "1f_Turn_To_Cube_6");
  chassis.MotionController.waitUntilSettled();
}
